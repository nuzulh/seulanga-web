...
const Header = () => {
  ...
  useEffect(() => {
    gsap.from(header, { y: -20, duration: 0.8, ease: "sine", opacity: 0 });
  }, []);
    ...
    const onScroll = () => {
      ...
      if (scroll > 100) {
        document
          ...
          .classList.add("bg-[white]", "shadow-lg", "duration-300");
      } else {
        document
          .querySelector("header")
          .classList.remove("bg-[white]", "shadow-lg");
      }
    };
...
<<<<<<< HEAD
  return (
    <header
      ref={(el) => (header = el)}
      className="fixed z-50 w-full flex items-center justify-between px-2 xl:px-24 py-2 font-[poppins]"
    >
      <div className="flex items-center gap-2">
        <img src={logo} className="w-8 sm:w-10" alt="" />
        <h1 className="text-xl font-bold tracking-wide text-[#334C0F]">
          Seulanga
        </h1>
      </div>
      <div className="hidden md:flex space-x-12">
        <NavLink
          to="/"
          className={({ isActive }) =>
            isActive ? "border-b border-[black]" : "hover:opacity-[0.7]"
          }
        >
          Home
        </NavLink>
      ...
      <i
        className={
          active
            ? "fa-solid fa-xmark p-3 text-xl md:hidden"
            : "fa-solid fa-bars p-3 text-xl md:hidden"
        }
        onClick={activeToggler}
      ></i>
      <div
        ref={(el) => (nav = el)}
        className="md:hidden flex flex-col items-center justify-center ease-out duration-500 absolute -left-full top-0 bg-[rgba(255,255,255,0.95)] text-[black] h-screen w-2/3"
      >
        <NavLink
          className="w-full flex items-center py-4 border-b-[1px] border-[rgba(0,0,0,.2)] justify-center"
          to="/"
        >
          Home
        </NavLink>
